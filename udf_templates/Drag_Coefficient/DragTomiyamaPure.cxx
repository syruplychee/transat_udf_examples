...
void template_class_name::operator()(double &ff, double &ffD, double vel_slip_mag, double Re, double Eo) {
   ...
   // Define some constants needed in the expression for tomiyama pure
   double Re_const = Re / (vel_slip_mag + _small);
   double constA = pow(Re_const, 0.687);
   double constB = Re_const * Eo / (9 * (Eo + 4));
   double fun0   = (2./3) + 0.1 * pow(Re, 0.687);
   double v_crit = pow(((4./3)/(0.1*constA)),(1/0.687));
   double ff1_blend = 0.5 + 0.5 * std::tanh(5*(vel_slip_mag-v_crit)/v_crit);
   double ff1 = ff1_blend*2 + (1-ff1_blend)*fun0;
   ...
   if (ff1 > ff2) {
      fun1 = 0.0687 * constA * pow(std::abs(vel_slip_mag), -0.313);
      fun2 = 2.5*(1-v_crit)/v_crit / (pow(std::cosh(5*(vel_slip_mag-v_crit)/v_crit), 2) + _small);
      ffD  = vel_slip_mag / (std::abs(vel_slip_mag) + _small) * (fun2*2 + fun1 - fun1*ff1_blend - fun0*fun2);
   } else {
...
void template_class_name::operator()(double &ff, double &ffDx, double &ffDy, double &ffDz, 
   ...
   // (in each direction)
   if (ff1 > ff2) {
      fun1 = 0.0687 * constA * pow(vel_slip_mag, -0.313);
      fun2 = 2.5*(1-v_crit)/v_crit / (pow(std::cosh(5*(vel_slip_mag-v_crit)/v_crit), 2) + _small);
